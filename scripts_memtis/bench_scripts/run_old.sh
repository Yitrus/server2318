...
# 这个脚本可以将数据分散到4个节点，d但是由于少了一个节点的显示，内存分配并不是期待的样子
# 作为一个版本备份，不再使用这个脚本
...
function func_main() {
	...
	sudo ${DIR}/bench_scripts/set_mem_size.sh htmm 1 ${NODE1}
	sudo ${DIR}/bench_scripts/set_mem_size.sh htmm 2 ${NODE2}
	# 这个节点3不能设计了，所有剩下的溢出的都放这里安全
	# sudo ${DIR}/scripts/set_mem_size.sh htmm 3 ${BENCH_DRAM}
